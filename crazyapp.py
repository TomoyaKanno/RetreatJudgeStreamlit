...
def assign_judges(posters, judges, reviews_per_poster, judge_availability):
    """
    Assign judges to posters in a load-balanced way, considering judge availability.
    For each poster, select eligible judges (those not from the same lab and available for that day/session)
    ...
    for idx, poster in posters.iterrows():
        ...
        # Filter judges who are available for this day and session
        available_judges = [
            judge for judge in judges['Name'] 
            if judge_availability[judge].get((poster_day, poster_session), True)
        ]
        
        # Try to exclude judges from the same lab.
        eligible_judges = [
            judge for judge in available_judges 
            if judges[judges['Name'] == judge]['Lab'].iloc[0] != poster_lab
        ]
        
        if len(eligible_judges) < reviews_per_poster:
            # If not enough eligible judges, use all available judges (regardless of lab).
            eligible_judges = available_judges
        
        # If still not enough, show an error
        if len(eligible_judges) < reviews_per_poster:
            error_message = (
                f"Error: Not enough judges available for poster '{poster['Poster_Title']}' "
                f"on {poster['Day']} {poster['Session']} at Board {poster['Board']}. "
                f"Required {reviews_per_poster} judges, but only {len(eligible_judges)} were found. "
                "Please add more judges or adjust their availability."
            )
    ...
    for judge, assignments in judge_assignments.items():
        assignment_strs = [
            f"{a['Day']} {a['Session']} (Board {a['Board']})" for a in assignments
        ]
...
def generate_excel(poster_assignments_df, judge_assigments_df, presenters_df, judges_df, judge_availability):
    """
    Generate an Excel workbook (in memory) with six sheets:
     ...
     Sheet 4: "Judge Availability" (the availability settings for each judge)
     Sheet 5: "Original Presenter" (the original presenters dataframe)
     Sheet 6: "Original Judges" (the original judges dataframe)
    """
    output = BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # Write judge assignments
        judge_assigments_df.to_excel(writer, sheet_name="Judge Review Assignments", index=False)
        
        # Create and write judge availability sheet
        availability_data = []
        for judge, avail in judge_availability.items():
            row = {'Judge': judge}
            for day in ['Day 1', 'Day 2']:
                for session in ['AM', 'PM']:
                    key = (day, session)
                    row[f"{day} {session}"] = "Available" if avail.get(key, True) else "Unavailable"
            availability_data.append(row)
        
        availability_df = pd.DataFrame(availability_data)
        availability_df.to_excel(writer, sheet_name="Judge Availability", index=False)
        ...
        for sheet_name in writer.sheets:
            ...
            # Additional grid formatting for Judge Schedule Grid and Availability
            if sheet_name in ["Judge Schedule Grid", "Judge Availability"]:
                ...
                for row in range(2, max_row + 1):
                    for col in range(1, max_col + 1):
                        ...
                        # Color coding for availability sheet
                        if sheet_name == "Judge Availability" and col > 1:
...
# Initialize session state for the app flow
if 'step' not in st.session_state:
    st.session_state.step = 'upload'  # Initial step is 'upload'

if 'presenters_df' not in st.session_state:
    st.session_state.presenters_df = None

if 'judges_df' not in st.session_state:
    st.session_state.judges_df = None

if 'judge_availability' not in st.session_state:
    st.session_state.judge_availability = {}

# Title of the app
st.title("Poster & Judge Assignment Tool")

# Step 1: Upload data and validate
if st.session_state.step == 'upload':
    st.markdown("""
    ### Step 1: Upload Data
    Upload the Excel file with poster presenter details and judge details.
    """)
        ...
        try:
            ...
            if st.button("Load Data"):
                ...
                if not required_poster_cols.issubset(presenters_df.columns):
                    ...
                    st.session_state.judges_df = judges_df
                    
                    # Initialize judge availability dictionary
                    judge_availability = {}
                    for judge in judges_df['Name']:
                        judge_availability[judge] = {
                            ('Day 1', 'AM'): True,
                            ('Day 1', 'PM'): True,
                            ('Day 2', 'AM'): True,
                            ('Day 2', 'PM'): True
                    ...
                    st.session_state.judge_availability = judge_availability
                    
                    # Move to the next step
                    st.session_state.step = 'manage_availability'
                    st.experimental_rerun()
        
        except Exception as e:
            st.error(f"Error reading the Excel file: {e}")

# Step 2: Manage judge availability
elif st.session_state.step == 'manage_availability':
    st.markdown("""
    ### Step 2: Manage Judge Availability
    Set the availability for each judge. By default, all judges are available for all sessions.
    Toggle the availability status for judges who are unavailable for specific sessions.
    """)
    
    # Configurable parameter for number of reviews per poster
    st.session_state.reviews_per_poster = st.number_input(
        "Number of Reviews per Poster", min_value=1, value=2, step=1
    )
    
    # Display the judge availability management UI
    st.subheader("Judge Availability")
    ...
    cols[0].markdown("**Judge**")
    cols[1].markdown("**Day 1 AM**")
    cols[2].markdown("**Day 1 PM**")
    cols[3].markdown("**Day 2 AM**")
    cols[4].markdown("**Day 2 PM**")
    
    # For each judge, create a row with toggles for each time slot
    for judge in st.session_state.judges_df['Name']:
        cols = st.columns([3, 2, 2, 2, 2])
        cols[0].write(judge)
        
        for i, (day, session) in enumerate(time_slots):
            key = f"{judge}_{day}_{session}"
            is_available = cols[i+1].toggle(
                "Available",
                value=st.session_state.judge_availability[judge].get((day, session), True),
                key=key
            )
            st.session_state.judge_availability[judge][(day, session)] = is_available
    
    # Add a "Reset All to Available" button
    if st.button("Reset All to Available"):
        for judge in st.session_state.judges_df['Name']:
            for day, session in time_slots:
                st.session_state.judge_availability[judge][(day, session)] = True
        st.experimental_rerun()
    ...
    if st.button("Generate Assignments"):
        try:
            ...
            original_presenters_df = st.session_state.presenters_df.copy()
            
            # Step 1: Assign poster boards
            presenters_df = assign_poster_boards(st.session_state.presenters_df.copy(), days=2)
            
            # Step 2: Assign judges using load balancing and considering availability
            poster_assignments_df, judge_assignments_df = assign_judges(
                presenters_df, 
                st.session_state.judges_df,
                st.session_state.reviews_per_poster,
                st.session_state.judge_availability
            )
            
            # Step 3: Generate Excel workbook
            excel_data = generate_excel(
                poster_assignments_df, 
                judge_assignments_df, 
                original_presenters_df, 
                st.session_state.judges_df,
                st.session_state.judge_availability
            )
            
            # Move to results step and store the results
            st.session_state.poster_assignments_df = poster_assignments_df
            st.session_state.judge_assignments_df = judge_assignments_df
    ...
    # Add a "Back" button to return to the upload step
...
# Step 3: Display results
    ...
    st.markdown("""
    ### Step 3: Review and Download Assignments
    The assignments have been generated successfully. Review the judge schedule matrix and download the Excel file.
    """)
    
    # Recreate judge_details dictionary for displaying the matrix
    judge_details = {}
    for _, row in st.session_state.poster_assignments_df.iterrows():
        for i in range(1, len([col for col in st.session_state.poster_assignments_df.columns if col.startswith('Judge_')]) + 1):
            judge_name = row[f'Judge_{i}']
            if judge_name not in judge_details:
                judge_details[judge_name] = []
            judge_details[judge_name].append({
    ...
    max_board = st.session_state.presenters_df['Board'].max()
    ...
    num_posters = len(st.session_state.presenters_df)
    num_judges = len(st.session_state.judges_df)
